...
module mod_chkdt
  ...
  subroutine chkdt_tw(nx,ny,nz,dxi,dyi,dzi,nh_d,nh_u,dzci,dzfi,u,v,w,dtmax)
    ...
    use mod_param, only: kappa1,kappa2,cp1,cp2
#endif
    !
    implicit none
    !
    ...
    !
#if defined(_HEAT_TRANSFER)
    dtth   = cfl_d*(max(kappa1/(rho1*cp1),kappa2/(rho2*cp2))*dlmini**2)**(-1)
...
#if defined(_HEAT_TRANSFER)
    use mod_param, only: kappa_sp,cp_sp
#endif
    !
    implicit none
    !
...
#if defined(_HEAT_TRANSFER)
    dtth   = cfl_d*((kappa_sp/(rho_sp*cp_sp))*dlmini**2)**(-1)
    dtmax  = min(dtmax,dtth)
#endif
    !
    return
