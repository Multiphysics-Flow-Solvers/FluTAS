...
module mod_mom
  ...
  contains
  !
  ! TODO: big GPU kernel, verify
  subroutine momad_xyz(nx,ny,nz,dxi,dyi,dzi,nh_d,nh_u,dzci,dzfi,u,v,w,mu,rho, &
                      dudt,dvdt,dwdt)  
    !
    implicit none
    !
...
#if defined(_OPENACC)
    integer :: istat
    attributes(managed) :: u,v,w,mu,rho,dudt,dzci,dzfi
#endif
    !    
    ...
    do k=1,nz
      do j=1,ny
        do i=1,nx
          !
#if defined(_OPENACC)
          !$acc cache(u(i-1:i+1,j-1:j+1,k-1:k+1))
          !$acc cache(v(i-1:i+1,j-1:j+1,k-1:k+1))
          !$acc cache(w(i-1:i+1,j-1:j+1,k-1:k+1))
          !$acc cache(mu(i-1:i+1,j-1:j+1,k-1:k+1))
          !$acc cache(rho(i-1:i+1,j-1:j+1,k-1:k+1))
#endif
          !
          ip = i + 1
          im = i - 1
